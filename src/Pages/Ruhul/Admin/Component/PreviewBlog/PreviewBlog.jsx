...
const PreviewBlog = () => {
    ...
    const currentItems = blogs && blogs?.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
                    ...
                    <tbody>
                        {currentItems && currentItems?.map((user, index) => (
                            <tr key={index} className="border-b dark:border-gray-600">
                                ...
                                <td className="p-4">
                                    <div className="text-gray-900 text-sm dark:text-gray-100">{user?.headline?.slice(0, 30)}...</div>
                                </td>
                                <td className="text-sm hidden md:flex">{user?.dateTime}</td>
                                <td>
                                    <button
                                        onClick={() => { setIsOpen(true); setData(user); }}
                    ...
                    >
                        ...
                        <div className="h-[70vh] overflow-y-auto">
                            ...
                            <div className="text-left">
                                <p className="font-semibold text-lg text-gray-900 dark:text-gray-100">{data.headline}</p>
                                <p className="my-3 text-gray-700 dark:text-gray-300">{data.dateTime}</p>
                                <p className="text-gray-800 dark:text-gray-200" dangerouslySetInnerHTML={{ __html: formattedDescription }} />
                            </div>
