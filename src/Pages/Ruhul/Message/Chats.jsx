...
const Chats = ({ reciver, sender, response }) => {
  ...
  return (
    <section className="flex flex-col h-full p-2 md:p-4 overflow-y-auto min-h-[65vh]">
          ...
          messages.map((message, index) => (
            <div key={index} className={`flex ${message.senderEmail === sender.email ? "justify-end" : "justify-start"}`}>
              <div className={`relative flex items-start ${message.senderEmail === sender.email ? "flex-row-reverse md:gap-3 gap-x-1" : "md:gap-3 gap-x-1"}`}>
                <img className="h-8 w-8 rounded-full object-cover md:h-10 md:w-10" src={message.senderPhoto} alt={message.senderEmail} />
