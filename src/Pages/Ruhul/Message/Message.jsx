...
import { FaSearch, FaArrowLeft, FaCrown, FaArrowRight } from "react-icons/fa";
...
const Message = () => {
  ...
  return (
    <section className="flex  flex-col md:flex-row h-[calc(100vh-56px)] relative">
      <Helmet>
      ...
      <button
        ...
        className="md:hidden fixed bottom-[13px] left-1 text-blue-500 rounded-full bg-transparent p-3 z-40"
      >
        <FaArrowLeft className="text-xl font-bold" />
      </button>

      <div
        className={`md:hidden  fixed inset-0 pt-16 bg-white dark:bg-gray-900 overflow-y-auto hide-scrollbar transition-transform z-40 ${
          drawerOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="p-4  flex-col items-center border-b border-gray-300 dark:border-gray-700">
        <div
          onClick={() => {
      ...
      <div className="hidden md:flex flex-col md:w-1/4 border-r p-4 bg-gray-100 dark:bg-gray-900 overflow-y-auto hide-scrollbar h-[calc(100vh-56px)]">
        <div
          onClick={() => {
            if (!canUseDropdown) openModal();
          }}
      ...
      <div className="w-full md:w-3/4 overflow-y-auto bg-white dark:bg-gray-900 h-[calc(100vh-56px)]">
